...
WAIT_DURATION_THRESHOLD = 5
CROSSING_GRACE_TIME = 2
DISPLAY_WIDTH = 1280
...
def yolo_detector_thread(video_source="Video/test_video4.mp4"):
    global annotated_frame, last_status, running
    ...
    while running:
        ...
        if ada_di_waiting_area:
            if waiting_start_time is None:
                ...
                time_green.append(current_time)
                no_detection_timer = None
        else:
            waiting_start_time = None
        ...
        if current_time - last_published_time >= mqtt_CD:
            mqtt_data = {
                ...
                "total_crossing": count_middle_crossing,
            }
            client.publish(MQTT_TOPIC, json.dumps(mqtt_data))
            with lock:
                last_status = mqtt_data
